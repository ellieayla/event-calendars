...
class PageInfo:
    ...
    def __repr__(self) -> str:
        nodef_f_vals = ((f.name, attrgetter(f.name)(self)) for f in fields(self) if attrgetter(f.name)(self) != f.default)
...
class PagedEdges[T]:
    ...
    def __init__(self, edges: list[EdgeCursor[T]], page_info: dict[str, Any], deserialize_with: Callable[[Any], T]):
        ...
        self.edges = list([deserialize_with(edge_cursor["node"]) for edge_cursor in edges])
    ...
    def from_prefetch(cls, group_data: dict[str, Any]) -> Self:
        ...
        return cls(
            ...
            past_events=PagedEdges(
                edges=group_data["past_events"]["edges"], page_info=group_data["past_events"]["page_info"], deserialize_with=Event.from_dict
            ),
            upcoming_events=PagedEdges(
                edges=group_data["upcoming_events"]["edges"], page_info=group_data["upcoming_events"]["page_info"], deserialize_with=Event.from_dict
            ),
        )
